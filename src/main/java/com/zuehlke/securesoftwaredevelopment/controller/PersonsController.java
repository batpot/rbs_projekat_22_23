...
public class PersonsController {
    ...
    public String person(@PathVariable int id, Model model, HttpSession session) {
        String csrd = session.getAttribute("CSRF_TOKEN").toString();
    ...
    public ResponseEntity<Void> person(@PathVariable int id) {
        ...
        AuditLogger.getAuditLogger(DatabaseAuthenticationProvider.class).audit("Obrisan je korisnik sa id-jem " + id);
    ...
    public String updatePerson(Person person, HttpSession session, @RequestParam("csrfToken") String csrfToken) throws AccessDeniedException {
        ...
        if(!csrf.equals(csrfToken)) {
            LOG.info("csrf napad na korisnika " + person.getId());
            throw new AccessDeniedException("Forbidden");
        }
        personRepository.update(person);
        LOG.info("Azurirani su podaci za korisnika '" + person.getFirstName() + " " + person.getLastName() + "'.");
        AuditLogger.getAuditLogger(DatabaseAuthenticationProvider.class).audit("Azuriraju se podaci za " + person.getId());
