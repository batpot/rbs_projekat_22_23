...
public class DatabaseAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if (success) {
            AuditLogger.getAuditLogger(DatabaseAuthenticationProvider.class).audit("Uspesan login za korisnika '" + username + "'");
            User user = userRepository.findUser(username);
            List<GrantedAuthority> grantedAuthorities = getGrantedAuthorities(user);
            return new UsernamePasswordAuthenticationToken(user, password, grantedAuthorities);
        }

        AuditLogger.getAuditLogger(DatabaseAuthenticationProvider.class).audit("Neuspesan login za korisnika '" + username + "'");
